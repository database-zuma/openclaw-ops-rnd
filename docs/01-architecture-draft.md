...
**Session 5 decision (lost sessions)**: Self-host PostgreSQL on dedicated KVM 2 VPS instead of Supabase. Full control, no external dependency, NocoDB for monitoring.
...
| No NocoDB-like UI for non-SQL monitoring | NocoDB as spreadsheet dashboard |
...
| Monitoring | NocoDB (Docker) at port 8080, connected to `openclaw_ops` |
...
|-------|--------|-------------|
| `raw.ddd_stock` | DDD warehouse stock | kode_barang, nama_gudang, kuantitas, snapshot_date |
| `raw.ddd_sales` | DDD warehouse sales | tanggal, kode_produk, kuantitas, harga_satuan, total_harga, bpp |
| `raw.ljbb_stock` | LJBB warehouse stock | Same pattern as ddd_stock |
| `raw.mbb_stock` / `raw.mbb_sales` | MBB warehouse | Stock + sales |
| `raw.ubb_stock` / `raw.ubb_sales` | UBB warehouse | Stock + sales |
| `raw.iseller_sales` | iSeller POS | TBD — iSeller API format |
...
| Deep (2 min) | NocoDB Table Editor (browse data, no SQL needed) | As needed |
