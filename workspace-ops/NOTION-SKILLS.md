# NOTION-SKILLS.md — Konvensi Notion untuk Atlas 🏔️

Kamu adalah **worker OPS**. Kamu update status task kamu sendiri di Notion setelah setiap job.
...
- ✅ **Query task milik kamu** — filter by Assigned Agent = "Atlas"
...
- ❌ **Jangan update task agent lain** (Apollo) — bukan urusanmu
...
curl -X POST 'https://api.notion.com/v1/databases/30031616-a0d5-81ac-86f0-e108677a6d0a/query' \
  ...
  -d '{
    "filter": {
      "and": [
        {"property": "Assigned Agent", "select": {"equals": "Atlas"}},
...
curl -X PATCH 'https://api.notion.com/v1/pages/TASK_PAGE_ID' \
  ...
  -d '{
    "properties": {
      ...
      "Notes": {"rich_text": [{"text": {"content": "Data updated. 1,397 SKU rows. Email sent to Mas Bagus."}}]}
    }
  }'
```
...
curl -X PATCH 'https://api.notion.com/v1/pages/TASK_PAGE_ID' \
  ...
  -d '{
    "properties": {
      ...
      "Last Run": {"date": {"start": "2026-02-07T08:30:00+07:00"}},
      "Notes": {"rich_text": [{"text": {"content": "GAGAL: Accurate API timeout setelah 30 detik. Error: connection refused."}}]}
    }
  }'
```
...
| Project | ID |
|---------|-----|
| Control Stock PoC | `30031616-a0d5-8110-9daa-f5ebf2d4ec34` |
